...
case class FunctionalPrinter(content: List[String] = Nil, indentLevel: Int = 0) {
  ...
  def printWithDelimiter[M](objects: Traversable[M], delimiter: String)(f: (M, FunctionalPrinter) => FunctionalPrinter): FunctionalPrinter =
    if (objects.size <= 1) print(objects)(f)
    else
      f(objects.last, objects.init.foldLeft(this) { (printer, obj) => f(obj, printer).add(delimiter) })
